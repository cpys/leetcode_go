...
func subdomainVisits(cpdomains []string) []string {
	...
	for _, cpdomain := range cpdomains {
		cpdomainUrl := strings.Fields(cpdomain)
		times, _ := strconv.Atoi(cpdomainUrl[0])
		url := cpdomainUrl[1]
		subdomains := strings.Split(url, ".")
		for subdomain, i := "", len(subdomains) - 1; i >= 0; i-- {
			if i == len(subdomains) - 1 {
				subdomain = subdomains[i]
			} else {
				subdomain = subdomains[i] + "." + subdomain
			}
			urlCount[subdomain] += times
		}
	}
	var ans []string
	for domain, times := range urlCount {
